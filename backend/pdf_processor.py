...
class PDFProcessor:
    ...
    def extract_text(self, pdf_path: str, progress_callback=None) -> str:
        ...
        if len(text.strip()) < 50 and self.fitz:
            fitz_text = self._extract_with_pymupdf(pdf_path, progress_callback)
            if len(fitz_text) > len(text):
                text = fitz_text
        
        # If still minimal text and OCR is enabled, try OCR
        if len(text.strip()) < 50 and self.enable_ocr and self.pytesseract:
            logger.info("Text extraction minimal, attempting OCR...")
            ocr_text = self._extract_with_ocr(pdf_path, progress_callback)
            if len(ocr_text) > len(text):
                text = ocr_text
        
        return self._clean_text(text)
    
    def _extract_with_pdfplumber(self, pdf_path: str, progress_callback=None) -> str:
    ...
    def _extract_with_pymupdf(self, pdf_path: str, progress_callback=None) -> str:
    ...
    def _extract_with_ocr(self, pdf_path: str, progress_callback=None) -> str:
